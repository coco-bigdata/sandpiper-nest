# sandpiper-nest插件开发宝典

本文面向sandpiper-nest插件开发人员，尝试尽可能全面地阐述开发一个sandpiper-nest插件所经过的历程，力求消除开发者的困惑，让插件开发变得简单。
...
当你打开这篇文档，想必已经不用在此解释什么是`sandpiper-nest`了。那下一个问题便是：

###  `sandpiper-nest`为什么要使用插件机制？

从设计之初，`sandpiper-nest`就把异构数据源同步作为自身的使命，为了应对不同数据源的差异、同时提供一致的同步原语和扩展能力，`sandpiper-nest`自然而然地采用了`框架` + `插件` 的模式：
...
- `Job`: `Job`是sandpiper-nest用以描述从一个源头到一个目的端的同步作业，是sandpiper-nest数据同步的最小业务单元。比如：从一张mysql的表同步到odps的一个表的特定分区。
...
为了简化对json的操作，`sandpiper-nest`提供了简单的DSL配合`Configuration`类使用。

`Configuration`提供了常见的`get`, `带类型get`，`带默认值get`，`set`等读写配置项的操作，以及`clone`, `toJSON`等方法。配置项读写操作都需要传入一个`path`做为参数，这个`path`就是`sandpiper-nest`定义的DSL。语法有两条：
...
为了规范源端和目的端类型转换操作，保证数据不失真，sandpiper-nest支持六种内部数据类型：
...
每个插件都必须在`sandpiper-nest`官方wiki中有一篇文档，文档需要包括但不限于以下内容：
...
4. **类型转换**
    - 插件是如何在实际的存储类型和`sandpiper-nest`的内部类型之间进行转换的。
    - 以及是否存在特殊处理。
5. **性能报告**
